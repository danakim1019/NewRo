...
LoadedModelData::LoadedModelData(string const& path, bool gamma = false, int k = 0) : gammaCorrection(gamma)
{
	type = k;
	loadModel(path);
}

float LoadedModelData::getDuration()
{
	float duration;
...
void LoadedModelData::setConstraints(float x, float y)
{
	m_constraints.push_back(Interval(x, y));
}

// draws the model, and thus all its meshes
void LoadedModelData::Draw(int meshNum, int num)
{
		meshes[meshNum].Draw(num);
}

int LoadedModelData::BoneTransform(float TimeInSeconds, vector<glm::mat4>& Transforms, vector<glm::fdualquat>& dqs)
{
...
void LoadedModelData::loadModel(string const& path)
{
...
void LoadedModelData::loadBones(aiNode* node, const aiScene* scene)
{
...
void LoadedModelData::processNode(aiNode* node, const aiScene* scene)
{
...
Mesh LoadedModelData::processMesh(aiMesh* mesh, const aiScene* scene)
{
	// data to fill
...
vector<Textures> LoadedModelData::loadMaterialTextures(aiMaterial* mat, aiTextureType type, string typeName)
{
	vector<Textures> textures;
...
void LoadedModelData::loadMeshBones(aiMesh* mesh, vector<VertexBoneData>& VertexBoneData)
{
...
void LoadedModelData::loadAnimations(const aiScene* scene, string BoneName, map<string, map<string, const aiNodeAnim*>>& animations)
{
...
void LoadedModelData::ReadNodeHeirarchy(const aiScene* scene, float AnimationTime, const aiNode* pNode, const glm::mat4& ParentTransform, const glm::fdualquat& ParentDQ, glm::vec3 startpos)
{
...
void LoadedModelData::CalcInterpolatedScaling(aiVector3D& Out, float AnimationTime, const aiNodeAnim* pNodeAnim)
{
...
void LoadedModelData::CalcInterpolatedRotaion(aiQuaternion& Out, float AnimationTime, const aiNodeAnim* pNodeAnim)
{
...
void LoadedModelData::CalcInterpolatedPosition(aiVector3D& Out, float AnimationTime, const aiNodeAnim* pNodeAnim)
{
...
unsigned int LoadedModelData::FindScaling(float AnimationTime, const aiNodeAnim* pNodeAnim)
{
...
unsigned int LoadedModelData::FindRotation(float AnimationTime, const aiNodeAnim* pNodeAnim)
{
...
unsigned int LoadedModelData::FindPosition(float AnimationTime, const aiNodeAnim* pNodeAnim)
{
